 ...
 * @author     Glen Langer (BugBuster)
 */
class ContaoFrontendController extends \Frontend implements JobTypesInterface
{
                    ...
                    if ($output != '') {
                        \System::getContainer()
                            ...
                            ->log(LogLevel::ERROR,
                                ...
                                ['contao' => new ContaoContext('ContaoFrontendController runJobs()', TL_ERROR)]);
                    } else {
                        \System::getContainer()
                            ->get('monolog.logger.contao')
                            ->log(LogLevel::ERROR,
                                'Cron job ' . $q->title . ' ' . ($cronJob['completed'] ? 'completed.' : 'processed partially.'),
                                ['contao' => new ContaoContext('ContaoFrontendController runJobs()', TL_GENERAL)]);
                    }
                } // if
            } else {
                $dataset = [
