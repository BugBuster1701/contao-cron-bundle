...
class StartJobsCron
{
	...
	private function runRouteJob($qjob)
	{
		...
		$url = \Contao\Environment::get('base') . ltrim($qjob['job'], '/');

		try
		{
			$request = new \BugBuster\Cron\CronRequest($url, (int) $qjob['expert_timeout']);
		}
		catch (\Exception $e)
		{
	...
	private function runUrlJob($qjob)
	{
		...
		try
		{
			$request = new \BugBuster\Cron\CronRequest($qjob['job'], (int) $qjob['expert_timeout']);
		}
		catch (\Exception $e)
		{
			$cronJob['completed'] = false;
